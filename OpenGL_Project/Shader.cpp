...
Shader::Shader(const char* vertex_file, const char* fragment_file, const char* geometry_file, int major_version, int minor_version)
{
	...
	vertex_shader = this->load_shader(GL_VERTEX_SHADER, vertex_file);
	fragment_shader = this->load_shader(GL_FRAGMENT_SHADER, fragment_file);
	if (geometry_file != "")
	{
		geometry_shader = this->load_shader(GL_GEOMETRY_SHADER, geometry_file);
	}
...
std::string Shader::load_shader_src(const char* file_name)
{
	...
	{
		std::cout << "COULD NOT LOAD SHADER SOURCE FILE!" << std::endl;
	}
	in_file.close();
